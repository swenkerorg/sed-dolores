...
[![Node CI Status](https://github.com/swenkerorg/sed-dolores/workflows/Node%20CI/badge.svg)](https://github.com/swenkerorg/sed-dolores/actions)
[![npm](https://img.shields.io/npm/v/@swenkerorg/sed-dolores.svg)](https://www.npmjs.com/package/@swenkerorg/sed-dolores)
[![codecov](https://codecov.io/gh/swenkerorg/sed-dolores/branch/master/graph/badge.svg)](https://codecov.io/gh/swenkerorg/sed-dolores)
...
```sh
npm install --save @swenkerorg/sed-dolores
```
...
```js
const { validator } = require('@swenkerorg/sed-dolores')
...
```js
const { parser } = require('@swenkerorg/sed-dolores')
...
`@swenkerorg/sed-dolores` supports the formats specified in JSON schema v4 (such as date-time).
...
When the `includeErrors` option is set to `true`, `@swenkerorg/sed-dolores` also outputs:
...
See the [doc/samples](./doc/samples/) directory to see how `@swenkerorg/sed-dolores` compiles
...
`@swenkerorg/sed-dolores` uses code generation to turn a JSON schema into javascript code that is easily
...
```sh
git clone https://github.com/swenkerorg/sed-dolores
cd schemasafe
...
Compared to `is-my-json-valid`, `@swenkerorg/sed-dolores` adds security-first design, many new features,
